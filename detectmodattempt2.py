...
class StreamThread(threading.Thread):
    ...
    def run(self):
        ...
        url = self.url
        device = self.device
        conf_thres = self.conf_thres = 0.25
        iou_thres = self.iou_thres = 0.45
...
def run():
    ...
    print("Menu")
    print("add source")
    uInput = input("Masukkan anda : ")
    if uInput == '1':
        source = input("Masukkan URL : ")
        is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS)
        is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))
        webcam = source.isnumeric() or source.endswith('.txt') or is_url
        if webcam or is_file:
            thread_num += 1
            stream_thread = StreamThread(str(thread_num), source, model, device)
            stream_thread.start()
        else:
